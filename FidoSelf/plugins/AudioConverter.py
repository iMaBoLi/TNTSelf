...
async def audioconverter(event):
    ...
    if not event.is_reply or not (event.reply_message.audio and event.reply_message.voice):
    ...
    await client.send_file(event.chat_id, video, attributes=attributes, progress_callback=callback, voice_note=voice)
    os.remove(audio)
    await event.delete()
