...
async def get_spector_buttons(event, userid, chatid):
    buttons = []
    for spec in SPECS:
        lists = event.client.DB.get_key(f"SPECTOR_{spec}") or []
...
async def Spector(event):
    ...
    res = await event.client.inline_query(event.client.bot.me.username, f"Spector:{chatid}:{event.userid}")
    if event.is_reply:
...
async def inlinespector(event):
    ...
    buttons = await get_spector_buttons(event, userid, chatid)
...
async def sendmessagespec(event):
    ...
    lists = event.client.DB.get_key("SPECTOR_SEND_MESSAGE") or []
    if event.sender_id in lists:
        ...
        if event.is_private:
            ...
            text = client.getstrings(STRINGS)["specsendmessage"].format(mention, event.sender_id, where, time)
        await event.client.bot.send_message(client.REALM, text)

@client.on(events.MessageEdited)
async def editmessagespec(event):
    if event.sender_id == event.client.me.id: return
    lists = event.client.DB.get_key("SPECTOR_EDIT_MESSAGE") or []
    if event.sender_id in lists:
        info = await event.client.get_entity(event.sender_id)
        mention = client.functions.mention(info)
        localtime = datetime.datetime.now()
...
async def statusspec(event):
    ...
    lists = event.client.DB.get_key("SPECTOR_STATUS") or []
    if event.user_id in lists:
        info = await event.client.get_entity(event.user_id)
        mention = client.functions.mention(info)
        localtime = datetime.datetime.now()
        time = localtime.strftime("%H:%M:%S")
...
async def actionspec(event):
    ...
    lists = event.client.DB.get_key("SPECTOR_ACTION") or []
    if event.user_id in lists:
        info = await event.client.get_entity(event.user_id)
        mention = client.functions.mention(info)
        localtime = datetime.datetime.now()
...
async def readpvspec(event):
    ...
    lists = event.client.DB.get_key("SPECTOR_READ_PV") or []
    if userid in lists:
        info = await event.client.get_entity(userid)
        mention = client.functions.mention(info)
        localtime = datetime.datetime.now()
...
async def joingroupspec(event):
    ...
    lists = event.client.DB.get_key("SPECTOR_JOIN_GROUPS") or []
    if userid in lists:
        info = await event.client.get_entity(userid)
        mention = client.functions.mention(info)
...
async def leavegroupspec(event):
    ...
    userid = (await event.get_user()).id
    if userid == event.client.me.id: return
    lists = event.client.DB.get_key("SPECTOR_LEAVE_GROUPS") or []
    if userid in lists:
        info = await event.client.get_entity(userid)
        mention = client.functions.mention(info)
        chat = await event.get_chat()
