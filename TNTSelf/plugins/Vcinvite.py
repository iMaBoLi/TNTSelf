...
async def invitevc(event):
    ...
    users = []
    async for user in event.client.iter_participants(event.chat_id, filter=types.ChannelParticipantsRecent, limit=50):
    ...
    invited = 0
    try:
        result = await event.client(functions.phone.InviteToGroupCallRequest(call=info.call, users=users))
        invited += 1
